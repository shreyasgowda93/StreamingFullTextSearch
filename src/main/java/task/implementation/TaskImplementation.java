...
public class TaskImplementation implements TaskInterface {
    ...
    private final ConcurrentHashMap<Types.QueryID, Query> activeQueries = new ConcurrentHashMap<>();
    ...
    public ErrorCode matchDocument(Types.DocID docId, String docStr) {
        ...
        executor.submit(() -> {
            ...
            for (Query query : activeQueries.values()) {
    ...
    private boolean isQueryMatchingDocument(Query query, String docStr) {
        ...
        for (String queryWord : queryWords) {
            boolean wordMatches = Arrays.stream(docWords).parallel().anyMatch(docWord -> {
